...
# **get**
> \Swagger\Client\Model\ResponseWrapperSalaryType get($id, $fields)
...
# **search**
> \Swagger\Client\Model\ListResponseSalaryType search($id, $number, $name, $description, $show_in_timesheet, $from, $count, $sorting, $fields)
...
try {
    $result = $apiInstance->search($id, $number, $name, $description, $show_in_timesheet, $from, $count, $sorting, $fields);
    print_r($result);
} catch (Exception $e) {
